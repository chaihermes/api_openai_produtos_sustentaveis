...
def gerar_recomendacao(parecer):
    ...
    conteudo = resposta.choices[0].message.content
    print("Finalizou a geração de recomendaçao")
    return conteudo


lista_de_transacoes = carrega("./dados/transacoes.csv")
transacaoes_analisadas = analisar_transacao(lista_de_transacoes)

for uma_transacao in transacaoes_analisadas["transacoes"]:
